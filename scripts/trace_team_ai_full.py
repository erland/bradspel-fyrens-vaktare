#!/usr/bin/env python3
"""
Stegloggning / trace för Fyrens väktare full Team-AI.

Syfte:
- Kör ett eller flera enskilda spel.
- Logga dag, spelare, position, uppdrag, handling, resurser, Mörker och byggprogress.
- Hjälpa oss se exakt var simulatorn tappar tempo.

Exempel:
  python scripts/trace_team_ai_full.py --players 3 --strategy mission_direct_build --seed 20260711
  python scripts/trace_team_ai_full.py --players 4 --strategy mission_opportunistic_ruin --seed 20260711 --max-days 12
"""
...
base_path = HERE / "simulate_team_ai_full.py"
spec = importlib.util.spec_from_file_location("simulate_team_ai_full_base", base_path)
base = importlib.util.module_from_spec(spec)
sys.modules["simulate_team_ai_full_base"] = base
...
def trace_one(players, strategy, seed, max_days=16, outdir="output/traces"):
    ...
    rows = []
    rows.append(snapshot(state, "START", note=f"strategy={strategy}, seed={seed}"))
    ...
    while not state.won and not state.lost and state.day <= max_days:
        ...
        for pi, p in enumerate(state.players):
            before_pos = p.pos
            before_carried = dict(p.carried)
            before_built = state.built
            before_darkness = state.darkness
            ...
            for ai in range(actions):
                ...
                if target is None:
                    action_note = "no target / build may have happened"
                elif p.pos != target:
                    if state.effects.get("blocked", 0) > 0:
                        ...
                        base.move(p, target)
                        action_note = f"move toward {target}"
                else:
                    ...
                    if p.mission.kind == "COLLECT" and p.mission.resource and p.carried.get(p.mission.resource, 0) >= p.mission.amount:
                        ...
                        action_note += " -> mission complete, switch to deliver"
        ...
        note = f"night; darkness {old_darkness}->{state.darkness}; hot +{state.hot_draws-old_hot}; base {resources_str(old_base)}->{resources_str(state.base)}"
        rows.append(snapshot(state, "NIGHT", note=note))
        state.day += 1
    ...
    base_name = f"trace-{players}p-{strategy}-seed{seed}"
    csv_path = out / f"{base_name}.csv"
    md_path = out / f"{base_name}.md"

    fieldnames = list(rows[0].keys())
    # ensure later p columns exist
    for row in rows:
        for k in row.keys():
    ...
    md_lines = [f"# Trace – {players}p {strategy} seed {seed}\n\n"]
    md_lines.append(f"Resultat: **{result}**. Fyrdelar byggda: **{state.built}**. Slut-Mörker: **{state.darkness}**. Dag: **{state.day}**.\n\n")
    md_lines.append("## Viktiga händelser\n\n")
    for row in rows:
        if row["event"] in ["START", "DAY_START", "NIGHT", "END"] or "BUILT" in row["note"] or "visited ruin" in row["note"] or "took crystal" in row["note"]:
            md_lines.append(
                f"- Dag {row['day']} {row['event']}"
                f"{' P'+str(row['player']) if row['player'] else ''}: "
                f"{row['part']}, byggt={row['built_parts']}, Mörker={row['darkness']}. {row['note']}\n"
            )

    md_lines.append("\n## Diagnos\n\n")
    if state.built == 0:
        md_lines.append("- AI:n byggde inte ens Grund. Kontrollera om sten samlas men inte levereras, eller om uppdrag nollställs för ofta.\n")
    elif state.built == 1:
        md_lines.append("- AI:n byggde Grund men fastnade före/under Torn. Kontrollera trä/sten-fördelning och leveranser till Fyrplatsen.\n")
    elif state.built == 2:
        md_lines.append("- AI:n nådde Ljuskärnan men vann inte. Kontrollera Grotta/kristalltempo och hotkortstryck.\n")
    else:
        md_lines.append("- AI:n vann. Jämför antal dagar och Mörker kvar med förlustspår.\n")
...
def main():
    ...
    ap.add_argument("--players", type=int, default=3, choices=[2,3,4])
    ...
    ap.add_argument("--outdir", default="output/traces")
    args = ap.parse_args()
